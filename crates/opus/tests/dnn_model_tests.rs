...
fn test_pitchdnn_multi_step_vs_c() {
    ...
    let n_steps = 5;
    let nb_if = opus_dnn::pitchdnn::NB_IF_FEATURES;
    ...
    for i in 0..n_steps {
        rust_result = opus_dnn::pitchdnn::compute_pitchdnn(
            &mut state,
...
fn test_rdovae_decode_vs_c() {
    ...
    let dec_arrays = opus_dnn::nnet::weights::parse_weights(&dec_blob).unwrap();
    let dec_model = opus_dnn::dred::rdovae_dec::init_rdovae_dec(&dec_arrays).unwrap();
...
fn test_fargan_single_frame_vs_c() {
    ...
    let arrays = opus_dnn::nnet::weights::parse_weights(&blob).unwrap();
    let model = opus_dnn::fargan::init_fargan(&arrays).unwrap();
    let mut state = opus_dnn::fargan::fargan_state_init(model);
    ...
    let mut rust_pcm = vec![0.0f32; opus_dnn::fargan::FARGAN_FRAME_SIZE];
    opus_dnn::fargan::fargan_synthesize(&mut state, &mut rust_pcm, &features);
...
fn test_fargan_multi_frame_vs_c() {
    ...
    let mut rust_pcm = vec![0.0f32; n_frames * opus_dnn::fargan::FARGAN_FRAME_SIZE];
    for i in 0..n_frames {
        let start = i * opus_dnn::fargan::FARGAN_FRAME_SIZE;
        let end = start + opus_dnn::fargan::FARGAN_FRAME_SIZE;
        opus_dnn::fargan::fargan_synthesize(
            &mut state,
